...
void printHelpExample()
{
  std::cout << "Inshimtu examples:"
            ...
            << "## Running \n"
               ...
               "``` \n"
               "\n"
               "Pre-existing files only: \n"
               "* Processes all existing files in build/testing matching the 'filename*.vti' shell glob;\n"
               "* Stops when all existing files are processed;\n"
               "* Uses the Catalyst script in gridviewer.py to transfer data to ParaView.\n"
               "``` \n"
               "build/Inshimtu -s testing/scripts/gridviewer.py -i build/testing/filename*.vti \n"
               "``` \n"
               "\n"
               "Variable Support: \n"
               ...
               "``` \n"
               "\n"
               "Inporter node specification: \n"
               "* Inport nodes import files and process them via Catalyst;\n"
               "* Inport nodes process files in parallel, extracting a subset of each file;\n"
               "* Use -n flag to specify list of nodes and node intervals that will be inport nodes.\n"
               "* The format of the inporter specification is: (#|#-#)[,(#|#-#)]*.\n"
               "``` \n"
               "build/Inshimtu -s testing/scripts/gridviewer.py -i build/testing/filename*.vti -n 0,2-4,16\n"
               "``` \n"
               "\n"
